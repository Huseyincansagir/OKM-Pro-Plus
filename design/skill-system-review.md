...
|---|---|---|---|---|---|
| `factory-erp-design-workflow` | Kodlama öncesi domain, UX, screen inventory, workflow ve Design Gate üretmek | Yeni ERP, fabrika, üretim/depo, public katalog, mobil operasyon veya kodlama öncesi tasarım talebi | Repository, mevcut docs, iş gereksinimi, design artefact'ları | `/design` dosyaları, decision log, implementation readiness | DISCOVER → DESIGN → DESIGN GATE |
| `factory-erp-architecture` | Tasarımı PostgreSQL, API, transaction, RBAC ve deployment mimarisine dönüştürmek | Design Gate başarılı ve schema/API/infra kararı gerektiğinde | `/design`, karar logu, açık kararların çözümü | Domain boundary, schema, migration planı, API contract, deployment planı | ARCHITECTURE |
